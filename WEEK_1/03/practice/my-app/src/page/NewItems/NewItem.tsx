interface INewItemProps {
  thumbnail: string;
  title: string;
  price: string;
  sale: string;
}

const NewItem = ({ thumbnail, title, price, sale }: INewItemProps) => {
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden max-w-md mx-auto p-4">
      <div className="relative w-full h-48 overflow-hidden rounded-md mb-4">
        <img
          src={thumbnail}
          alt={title}
          className="w-full h-full object-cover transition-transform duration-300 hover:scale-105"
        />
        {sale && (
          <span className="absolute top-2 right-1 bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded">
            {sale}
          </span>
        )}
      </div>
      <h3 className="text-lg font-semibold text-gray-900 mb-2">{title}</h3>
      <div className="text-sm text-gray-500 mb-2">{price}</div>
    </div>
  );
};

export default NewItem;
